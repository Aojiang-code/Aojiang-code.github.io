...
MFCC是一种常用的音频特征，它在语音和音乐处理中非常有用，因为它能够捕捉到音频信号的重要特性。函数首先初始化两个空列表，用于存储特征和标签。然后，它遍历每个子目录中的音频文件，使用`librosa`库加载音频数据，并计算MFCC特征。每个文件的MFCC特征和对应的标签都被添加到列表中。最后，函数返回这两个列表，它们可以用于训练机器学习模型。在实际应用中，你需要确保`librosa`库已经安装在你的系统上。
###### 详细介绍
